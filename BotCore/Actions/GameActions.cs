...
namespace BotCore.Actions
{
    public class GameActions
    {
        ...
        public static void Pickup(GameClient Client, Position Position)
        {
            int firstEmptySlot = -1;
            for (int i = 0; i < Client.GameInventory.Items.Length; i++)
                if (Client.GameInventory.Items[i] == null)
                {
                    firstEmptySlot = i;
                    break;
                }
            ...
            var p = new Packet();
            p.WriteByte(0x07);
            p.WriteByte((byte)(firstEmptySlot + 1));
            p.WriteInt16(Position.X);
            p.WriteInt16(Position.Y);
            p.WriteByte(0x00);
            p.WriteByte(0x07);
            GameClient.InjectPacket<ServerPacket>(Client, p);
        }
