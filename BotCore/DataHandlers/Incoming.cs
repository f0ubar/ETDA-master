...
namespace BotCore.DataHandlers
{
    public static class Incoming
    {
        public static void MapLoaded(object sender, Packet packet)
        {
            ...
            new Thread(delegate()
               {
                   LoadMap(client, number, width, height, name);
        ...
        public static void InitializeMapLoad(Client client)
        {
            ...
            var name = client.FieldMap.MapName;
            LoadMap(client, number, width, height, name);
        }

        private static void LoadMap(Client client, short number, short width, short height, string name)
        {
            var path = Path.Combine(Collections.DAPATH, "maps") + "\\lod" + number.ToString(CultureInfo.InvariantCulture) + ".map";

            if (!string.IsNullOrEmpty(name))
            {
                string SerializedMapInfo = Newtonsoft.Json.JsonConvert.SerializeObject(
                    new
                    {
                        Name = name,
                        Number = number,
                ...
                System.IO.File.WriteAllText(path + ".json", SerializedMapInfo);
            }

            if (File.Exists(path))
                PrepareMap(client, number, width, height, path, name);
        }

        private static void PrepareMap(Client client, short number, short width, short height, string path, string name)
        {
            client.FieldMap.Ready = false;
            client.FieldMap.OnMapLoaded(new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.ReadWrite), number, width, height, name);
